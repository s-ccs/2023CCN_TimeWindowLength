...
# ╔═╡ ee3e20c4-f964-11ed-1ee7-b79e32af2c9b
begin
	using PyMNE
	using Unfold
	using CairoMakie, MakieThemes, AlgebraOfGraphics, Colors, ColorSchemes
	using Statistics
	using DataFrames, DataFramesMeta, CSV
end

# ╔═╡ df016cb4-49d1-446d-9368-fe7a63e87260
set_theme!(theme_ggthemr(:fresh))

# ╔═╡ c3efa01b-384a-4b2e-8690-16173daaec98
function calc_time_models(evts,dat,tWinList,sfreq)
	mList = []
	for twindow = tWinList  
		m = fit(UnfoldModel,Dict(
				"fixation"=>(
					@formula(0~1),
					firbasis(twindow,sfreq,string(twindow))),
				"stimulus"=>(
					@formula(0~1),
					firbasis(τ=(-0.2,1),sfreq=sfreq,name="stimulus")),
		),evts,dat, eventcolumn="type");
		res = coeftable(m)
		push!(mList,res)
	end
	return vcat(mList...)
...
# ╔═╡ eafe353d-0c1a-4c4c-aeea-52ffa8b1a3c7
function basisToWin(basisname)
	return parse.(Float64,[s[2] for s in split.(replace.(basisname,"("=>"",")"=>""),',')])
end

# ╔═╡ 6de0914f-702f-4b10-b9cc-71a5af8b61a3
tWinList = [(-0.1,3),(-0.1,0.5),(-0.1,0.45),(-0.1,0.4),(-0.1,0.35),(-0.1,0.30),(-0.1,0.25),(-0.1,0.2),(-0.1,0.15), (-0.1,0.10), (-0.1,0.05)]

# ╔═╡ bfe63bbf-d336-4fc5-b649-1f568034bb17
sub = 23

# ╔═╡ 55dbc24b-2fe2-4c39-a224-657c320c8a42
begin
...
# ╔═╡ a959616b-9e26-48e9-a765-ad31ebda59e4
sfreq = pyconvert(Int64, raw.info["sfreq"]);

# ╔═╡ 7f0b58e4-174f-42c8-9e1b-57ba5b182316
begin
	#channel = "Oz" #Index 31;
	# @Anmol Note: you probably want all channels here, just remove the picks=... part
	channel_list = [31]
	dat = pyconvert(Array,raw.get_data(picks=channel_list)).*10^6;
end

# ╔═╡ df36e2d3-dcfd-471e-a197-17851878b5a4
begin
...
# ╔═╡ 2401cd6b-6cbd-41ff-9de0-56f1f8ba33d3
events

# ╔═╡ 1300bb97-2c90-4720-a5e8-48321842ec3d
size(@subset(events, :type .== "fixation"))

# ╔═╡ 7ac4b7ee-cae6-486a-8877-a14bfe4ff5c3
res = calc_time_models(events,dat,tWinList,sfreq)

# ╔═╡ aef0947b-70cc-4f78-b0a0-03db482f1562
begin
	resSub = @subset(res,:basisname .!= "stimulus")
	resSub.tWin .= 	basisToWin(resSub.basisname)
end;

# ╔═╡ e42b7318-9a0b-4048-bf6b-14fb050ae834
unique(resSub.basisname)

# ╔═╡ dc3f7e9b-2983-4809-aa10-aeb768cc9ec6
let
	
	f = Figure()
	

	h_t = data(@subset(resSub,:basisname .!= "(-0.1, 3)")) * mapping(:time,:estimate,color=:tWin,group=(:tWin,:coefname)=>(x,y)->string(x)*y)

	
	untWin = unique(resSub.tWin)
	segDF = 	DataFrame(
	...
	h1 = visual(Lines,colormap=get(ColorSchemes.Reds,0.3:0.01:1)) * (h_l + h_t) |> x->draw!(f[1,1],x,axis=(;xlabel="time [s]",ylabel="estimate [μV]"))
...
# ╔═╡ 5202e94c-5198-4ced-accc-a203891cc147
let
	d3 = @subset(resSub,:basisname .!= "(-0.1, 3)")
	f = Figure()
	ax = Axis(f[1,1])
	for b = unique(d3.basisname)
		#@show b
		bsl = @subset(d3, :basisname .== b, :time .< 0)
		time = d3[d3.basisname .== b,:].time
		tmp = d3[d3.basisname .== b,:].estimate .- mean(bsl.estimate)
		lines!(ax,time,tmp)
	end
	current_figure()
end

# ╔═╡ b668706a-2e46-4376-9a04-42f3440477bb
let
	f = Figure()
	tWinLong = [(-0.1,x) for x in [3,2.5,2,1.5,1,0.5]]
	#tWinLong = reverse(tWinLong)
	dFull = calc_time_models(events,dat,tWinLong,sfreq)
	d = @subset(dFull, :basisname .!= "stimulus")
	...
	visual(Lines,colormap=reverse(get(ColorSchemes.Blues,0.3:0.01:1.2))) * (h_l + data(d)*mapping(:time,:estimate,color=:tWin,group=:tWin=>nonnumeric))|>x->draw!(f,x,axis=(;xlabel="time [s]",ylabel="estimate [μV]"))
...
# ╔═╡ ff11c5fb-557b-444d-8743-8de341cf9f45
begin
	tWinL = [(-0.1,x) for x in [3,2.5,2,1.5,1,0.5]]

	dF = calc_time_models(events,dat,tWinL,sfreq)

	d1 = @subset(dF, :basisname .!= "stimulus")
	d1.tWin = basisToWin(d1.basisname)
	d2 = d1
end

# ╔═╡ c1f3356c-3e0d-4dfe-81bd-7f50d2e655c2
let
	x = "(-0.1, 3.0)"
bsl = @subset(d1, :basisname .== x, :time .< 0)
mean(bsl.estimate)
y = d1[d1.basisname .== x,:].estimate .- mean(bsl.estimate)
	lines(d1[d1.basisname .== x,:].estimate)
	lines!(y)
	current_figure()
end

# ╔═╡ b4e11e7f-b908-4ddb-918c-1fb664956d11
begin

	for b = unique(d1.basisname)
		@show b
		bsl = @subset(d1, :basisname .== b, :time .< 0)
		
		tmp = d1[d1.basisname .== b,:].estimate .- mean(bsl.estimate)
		d1[d1.basisname .== b,:].estimate = tmp
	end

end


# ╔═╡ eff64272-a96a-4e69-a1fd-e72305393e41
begin
	lines(d1[d1.basisname .== "(-0.1, 1.0)",:].estimate)
	lines!(d2[d1.basisname .== "(-0.1, 1.0)",:].estimate)
	current_figure()
end

# ╔═╡ d806201e-28cd-4a0c-952d-9bb591ca6363
let
	d3 = d1
	...
	for b = unique(d3.basisname)
		...
		tmp = d3[d3.basisname .== b,:].estimate .- mean(bsl.estimate)
		d3[d3.basisname .== b,:].estimate = tmp
	end
	
	f = Figure()
	untWin = unique(d3.tWin)
	segDF = 	DataFrame(
		:x=>hcat(repeat([-0.1],length(untWin)),untWin)'[:],
		:y=>repeat(reverse(1:length(untWin)),inner=2))
	segDF.tWin .= 0.
	...
	visual(Lines,colormap=reverse(get(ColorSchemes.Blues,0.3:0.01:1.2))) * (h_l + data(d3)*mapping(:time,:estimate,color=:tWin,group=:tWin=>nonnumeric))|>x->draw!(f,x,axis=(;xlabel="time [s]",ylabel="estimate [μV]"))
	#lines!(time_gt,dat_gt[:,1],color=Colors.Gray(0.2),linewidth=3)
...
# ╔═╡ 80e96ed4-47d0-4ada-81da-97292e0627fb
let
	d3 = d1
	f = Figure()
	ax = Axis(f[1,1])
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "7730ab0406633baa6cd810fe4a944cfc508492f0"
